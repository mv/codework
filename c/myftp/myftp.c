/*
** $Id: myftp.c 6 2006-09-10 15:35:16Z marcus $
**
** myftp client
**
...
int send_fnm( int fd, char *file_name );

int main (int argc, char *argv[])
{
    ssize_t     rd, wr;
    int         sockfd, n, i, total=0, bytes=0;
    socklen_t   len;
    char        buff[READ_SIZE];
    ...
    /* Envia ao server o nome de arquivo desejado */
    /***
    bytes = send_fnm( sockfd, filename );
    l4c_log("    File size on server: %d bytes.\n");
    if( bytes < 0 )
    {
        err_quit( "File NOT FOUND on server.\n" );
    }
    /***/
    ...
    /****/
    while ( (rd = Read(sockfd, buff, 1)) > 0)
    {
        wr = fwrite( buff, 1, 1, fh );
        total += wr ;
...
int send_fnm( int fd, char *file_name )
{
    ...
    snprintf(filename, sizeof(FILENAME), "fnm=(%s)\n", file_name);
    Write( fd, filename, FILENAME );
    return 0;
}
