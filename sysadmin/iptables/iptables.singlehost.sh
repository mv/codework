...
# Accept http/https connections from the Internet
$IPT -A INPUT -i $WAN -p tcp --dport 80   -j ACCEPT
$IPT -A INPUT -i $WAN -p tcp --dport 443  -j ACCEPT
$IPT -A INPUT -i $WAN -p tcp --dport 8000 -j ACCEPT
$IPT -A INPUT -i $WAN -p tcp --dport 8080 -j ACCEPT
$IPT -A INPUT -i $WAN -p tcp --dport 8088 -j ACCEPT
$IPT -A INPUT -i $WAN -p tcp --dport 8090 -j ACCEPT

# Oracle
$IPT -A INPUT -i $LAN -p tcp -s 10.11.32.0/24    --dport 1521  -j ACCEPT
$IPT -A INPUT -i $LAN -p tcp -s 192.168.101.0/24 --dport 1521  -j ACCEPT
$IPT -A INPUT -i $LAN -p tcp -s 172.16.30.0/24   --dport 1521  -j ACCEPT

# PostGres
$IPT -A INPUT -i $LAN -p tcp -s 10.11.32.0/24    --dport 5432  -j ACCEPT
$IPT -A INPUT -i $LAN -p tcp -s 192.168.101.0/24 --dport 5432  -j ACCEPT
$IPT -A INPUT -i $LAN -p tcp -s 172.16.30.0/24   --dport 5432  -j ACCEPT

# Samba
# http://hr.uoregon.edu/davidrl/samba.html#samba_security
# Allow access to Samba from a particular subnet
iptables -A INPUT -p icmp -s 192.168.101.0/24                               -j ACCEPT
iptables -A INPUT -p udp  -s 192.168.101.0/24              --dport  137:138 -j ACCEPT
iptables -A INPUT -p tcp  -s 192.168.101.0/24 -m multiport --dports 139,445 -j ACCEPT
iptables -A INPUT -p icmp -s 10.11.32.0/24                                  -j ACCEPT
iptables -A INPUT -p udp  -s 10.11.32.0/24                 --dport  137:138 -j ACCEPT
iptables -A INPUT -p tcp  -s 10.11.32.0/24    -m multiport --dports 139,445 -j ACCEPT
