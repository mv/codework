...
describe SqlParser, :shared => true do
  before do
...
describe SqlParser, "parsing" do
  it_should_behave_like "SqlParser"
  
  it "when conditions are not joined with an :and or :or, does not succeed" do
    @parser.parse("select first_name from users where first_name='joe' last_name='bob'").should_not be_success
  end
end
