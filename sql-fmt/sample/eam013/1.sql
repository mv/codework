SELECT ewo.organization_id
 ...
 WHERE wt.employee_id                               = func.person_id
   ...
   AND ewo.scheduled_start_date               BETWEEN NVL(      TO_DATE(&l_dt_ini_ini)          , ewo.scheduled_start_date)
                                                  AND NVL(TRUNC(TO_DATE(&l_dt_ini_fim)) + .99999, ewo.scheduled_start_date)
   ...
   AND db.department_code                     BETWEEN NVL(&p_depart_atrib_ini                   , db.department_code)
                                                  AND NVL(&p_depart_atrib_fim                   , db.department_code)
